  ...
  *registration = nullptr;
  ...
  if (builtin_code > BuiltinOperator_MAX) {
    TF_LITE_REPORT_ERROR(
        ...
        "with newer model?",
        builtin_code);
    Serial.println("Op builtin_code out of range");
    status = kTfLiteError;
    ...
    *registration = op_resolver.FindOp(builtin_code, version);
    if (*registration == nullptr) {
      ...
      Serial.println("Didn't find op for builtin opcode");
      status = kTfLiteError;
    }
    ...
    Serial.println("Operator with CUSTOM builtin_code has no custom_code");
    status = kTfLiteError;
  } else {
    ...
    *registration = op_resolver.FindOp(name, version);
    if (*registration == nullptr) {
      ...
      // while preparing ops.
      Serial.println("Didn't find op for custom opcode");
      status = kTfLiteError;
    }
  }
  return status;
